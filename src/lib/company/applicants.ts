...
export async function getCompanyApplicantDetail(
  ...
  const [
    ...
    supabase
      .from("engineer_profiles")
      .select(
        "prefecture, years_of_experience, self_pr, work_style, portfolio_url, job_title, job_category, availability_status, github_url, desired_annual_income_yen, desired_hourly_rate_min, desired_hourly_rate_max, available_from",
      )
      .eq("id", applicantId)
      .maybeSingle(),
